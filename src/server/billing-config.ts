 ...
 *
 * BILLING_MODE gates whether we talk to the real payment provider (PayPal, "live") or run an
 * internal stand-in ("mock", the default). Mock mode lets the whole billing flow —
 * checkout → trial → cancel → addon — be built and verified with no PayPal account. This
 * module holds only the bits that are provider-agnostic so the mock routes don't have to
 * import from the PayPal client.
 */
